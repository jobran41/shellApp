...
module.exports = {
 ...
 function   createFile(req, res, next) {
  ...
  /*     exec(`git status && git add . && git commit -m "add file"`, (error, stdout, stderr) => {
    ...
    if (await File.findOne({ nameMachine: nameMachine })) {
      throw 'nameMachine "' + nameMachine+ '" is already taken';
  }

  const file = new File(req.body);
  console.log(file,"The fil");
...
async function create(fileParam) {
    console.log(fileParam,"userparam")
    // validate
    if (await File.findOne({ username: userParam.username })) {
...
async function update(id, userParam) {
    ...
    if (user.username !== userParam.username && await File.findOne({ username: userParam.username })) {
        throw 'Username "' + userParam.username + '" is already taken';
    }
